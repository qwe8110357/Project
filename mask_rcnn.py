...
class MaskRCNN:
    ...
    def draw_object_info(self, bgr_frame, depth_frame, color_frame,Odepth_frame,prereal_coordinate):
        # loop through the detection
        
        real_coordinate=np.zeros((3,90))
        for box, class_id, obj_center in zip(self.obj_boxes, self.obj_classes, self.obj_centers):
            ...
            depth_intrin = Odepth_frame.profile.as_video_stream_profile().intrinsics
            #camera_coordinate
            camera_coordinate=rs.rs2_deproject_pixel_to_point(depth_intrin,[cx,cy],depth_mm )
            camera_coordinate=np.asarray(camera_coordinate, dtype = int)/10
            camera_coordinate=np.multiply(camera_coordinate,fix)
            real_coordinate[0][i]=camera_coordinate[0][0]
            real_coordinate[1][i]=camera_coordinate[0][1]
            real_coordinate[2][i]=camera_coordinate[0][2]
            class_name = self.classes[int(class_id)]
            velocity=np.sum((real_coordinate[:,i]-prereal_coordinate[:,i])**2)
            ...
            cv2.putText(bgr_frame, "{} cm".format(real_coordinate[:,i]), (x + 5, y + 60), 0, 1.0, (255, 255, 255), 2)
            cv2.putText(bgr_frame, "{} cm".format(prereal_coordinate[:,i]), (x + 5, y + 90), 0, 1.0, (255, 255, 255), 2)
            cv2.putText(bgr_frame, "{} m/s".format('%.3f'%velocity), (x + 5, y + 120), 0, 1.0, (255, 255, 255), 2)
        ...
        return bgr_frame,real_coordinate
